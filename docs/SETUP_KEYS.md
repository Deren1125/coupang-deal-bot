...
- `open_markets`(G마켓·옥션·11번가·롯데온·SSG·알리·테무·오늘의집·네이버·토스처럼 누구나 파는 몰)의 딜은 제목·게시글·판매자명에 `official_markers`(공식, 브랜드관, 직영 등)가 있어야 자동으로 올립니다. 없으면 관리자 챗에 **"🔎 정품 확인 필요 #번호"** 가 오고, 상품 페이지를 보고 `/ok 번호` 하면 그때 링크를 만들어 올립니다(`/skip 번호` 는 안 올림). 링크를 직접 만드는 몰(토스·네이버)은 링크 요청 메시지에 확인 문구가 붙습니다.
...
- 봇이 오픈마켓 상품 페이지를 못 읽는 경우(차단)가 있어 "확인 필요"가 자주 올 수 있습니다. 귀찮으면 `unverified_action: skip` 으로 바꾸면 확인 없이 건너뜁니다.
