...
"Instagram API with Instagram Login" 을 씁니다. 페이스북 페이지는 필요 없고, 내 계정만 쓰면 앱 검수도 필요 없습니다.

1. 인스타 앱 → 설정 → 계정 유형 및 도구 → **프로페셔널 계정으로 전환** (크리에이터 또는 비즈니스, 무료). 개인 계정은 API 게시가 안 됩니다.
2. developers.facebook.com → (스레드에 쓰는) 그 앱 → 왼쪽 **사용 사례 → 사용 사례 추가 → "Instagram에서 메시지 및 콘텐츠 관리"** (Instagram API with Instagram Login) 추가.
3. 왼쪽 메뉴 **Instagram → API 설정(Instagram 로그인)** 화면:
   - **1. 인스타그램 앱 ID / 인스타그램 앱 시크릿** → Railway Variables 에 `INSTAGRAM_APP_ID`, `INSTAGRAM_APP_SECRET` (스레드·메타 앱 ID 와 또 다른 번호입니다).
   - **2. Instagram 테스터 추가**(앱이 개발 모드일 때) → 내 계정 추가 → 인스타 앱 → 설정 → 웹사이트 권한 → 앱 및 웹사이트 → **테스터 초대 수락**.
   - **3. 비즈니스 로그인 설정** → **OAuth 리디렉션 URI** 에 `https://<Railway 도메인>/instagram/callback` (관리자 챗 `/instaauth` 메시지 1)에 정확한 주소가 나옵니다). 제안을 눌러 확정하고 저장. 7-1 의 4번처럼 Railway 도메인이 먼저 있어야 합니다.
4. Redeploy → 관리자 챗 `/instaauth` → 링크를 길게 눌러 복사해 브라우저에서 열고 승인 → "✅ 인스타그램 연결 완료" 가 오면 끝. (수동이면 주소창 `code=` 값을 `/instacode 값`.)
5. `/instatest` 로 샘플을 실제로 올려 카드 이미지와 본문을 확인하고 앱에서 지우세요. `/status` 의 📸 줄이 ✅ 이면 다음 딜부터 자동으로 올라갑니다.

알아 둘 것:
- 인스타는 **사진 없는 글을 못 올리고 JPEG 만** 받습니다. 그래서 봇이 딜마다 **카드 이미지**(상품 사진 + 상품명·가격·할인율, 1080×1350)를 만들어 `https://<도메인>/media/…` 로 서빙하고 그걸 올립니다. 카드 문구는 `config.yaml` 의 `card` 에서, 글꼴은 도커 이미지의 나눔고딕을 씁니다.
- 본문의 링크는 **눌리지 않습니다.** 그래서 본문은 "구매 링크는 프로필 링크에서" 로 유도하고, 인스타 프로필 링크에는 봇의 **핫딜 페이지** `https://<도메인>/` 를 넣으세요. 최근 48시간 딜이 구매 버튼과 함께 나오고(`web` 설정), 각 딜은 `https://<도메인>/d/번호` 로 바로 구매 링크로 넘어갑니다. 수수료 고지는 본문과 페이지 둘 다에 들어갑니다.
- 정보 글(이벤트류)은 기본으로 인스타에 안 올립니다 (`instagram.info_posts`). 하루 100건 제한, 비용 없음.
- 토큰(60일)은 만료 전에 자동 갱신됩니다. 스레드는 상품 사진이 없거나 메타가 거절하면 같은 카드 이미지를 대신 씁니다.
...
## 7-4. 정품·공식 판매처 확인, 중복 차단

**정품 확인(필수 기준).** `deal.authenticity` 설정대로 발행 직전에 판단합니다.
- 제목·게시글 본문·상품 페이지 어디에든 `reject_keywords`(병행수입, 구매대행, 리퍼, 중고 등)가 보이면 올리지 않습니다.
- `open_markets`(G마켓·옥션·11번가·롯데온·SSG·알리·테무·오늘의집·네이버·토스처럼 누구나 파는 몰)의 딜은 제목·게시글·판매자명에 `official_markers`(공식, 브랜드관, 직영 등)가 있어야 자동으로 올립니다. 없으면 `unverified_action` 대로: **`skip`(기본) = 묻지 않고 제외**, `review` = 관리자 챗에 "🔎 정품 확인 필요 #번호" 가 오고 `/ok 번호` 하면 그때 올림(토스·네이버는 링크 요청 메시지에 확인 문구가 붙음), `allow` = 그냥 올림.
- `skip` 으로 바꾸고 재시작하면 그때까지 확인을 기다리던 딜도 자동으로 내려갑니다 (정보 글 확인 요청은 그대로).
- **식품**(`deal.food` 분류)은 '정품' 개념이 없으니 판매처 확인이 안 돼도 통과합니다. 병행수입·중고 같은 거부 낱말은 식품에도 그대로 적용됩니다.
- 공식 몰·직영 몰(올리브영, 컬리, 무신사, LF몰, H패션몰, 아디다스 등)은 병행수입 표시만 없으면 통과합니다.
- 후기가 `trusted_reviews`(기본 100)건 이상 쌓인 상품은 공식 표시가 없어도 통과합니다(오래 팔린 판매처). 쿠팡은 판매자를 봇이 가려내지 못하므로 후기 양으로 봅니다: 후기 수를 알았는데 `coupang_min_reviews`(기본 30)건보다 적으면 올리지 않고, 후기 수를 모르면 통과합니다.
- 봇이 오픈마켓 상품 페이지를 못 읽는 경우(차단)가 있어 `review` 로 두면 "확인 필요"가 자주 옵니다. 잠깐씩만 볼 수 있으면 기본값 `skip` 이 낫습니다. 놓치는 딜이 아깝다면 `review` 로.

**중복 차단.** 같은 상품 ID 는 `publish.dedup_days` 안에 다시 올리지 않고, ID 가 달라도(다른 게시판·주소 변형) 이름 유사도가 `dedupe_similarity` 이상이면 같은 딜로 보고 건너뜁니다. 숫자(모델·용량·수량)가 다르면 다른 상품으로 봅니다. 전에 올린 것보다 `dedupe_cheaper_pct`% 이상 싸졌으면 새 딜로 인정합니다. 대기 중인 글과 겹쳐도 넣지 않습니다.

## 7-5. 관리자 챗 한 번에 답하기

관리자 챗 맨 위 고정 공지(📌)에 **내 링크가 필요한 글**과 **확인해 줘야 하는 글**(정품 확인, 정보 글)이 생길 때마다 쌓입니다. 답하지 않으면 계속 남아 있고(링크·확인 대기는 `manual_link_ttl_hours` 뒤 만료), 처리하면 공지가 새로 갱신됩니다. 답은 그 공지에 답장(또는 그냥 메시지)으로 **한 줄에 하나씩** 보내면 됩니다.

```
675 https://toss.im/share/…      ← 링크 붙이기
677 ok                            ← 정품 확인·정보 글 그대로 올리기
673 skip                          ← 안 올리기
```

여러 줄을 한 번에 보내도 되고, `/link` `/ok` `/skip` 명령과 요청 메시지에 답장하는 방식도 그대로 됩니다.

## 7-6. 글의 강조 단계와 식품 기준

- 채널·카톡·블로그 글은 이모지를 글마다 최대 2개만 쓰고, "평균가 대비 N% 저렴" 같은 말은 쓰지 않습니다. 대신 평소 가격(30일 평균·쿠팡 시중가) 대비 `publish.emphasis_must_pct`(50%) 이상 싸면 "꼭 담으세요", `emphasis_top_pct`(70%) 이상이면 "역대급" 문구가 붙고, 그 미만은 짧게만 강조합니다.
- 식품류(`deal.food`: 분류나 상품명으로 판단)는 평소 가격 대비 `min_below_reference_pct`(50%) 이상 쌀 때만 특가로 봅니다. 표시 할인율이나 추천 수만으로는 올리지 않습니다. 그 외 상품은 기존 기준 그대로입니다.
...
**하루 정리 블로그 글.** 매일 밤 `blog_digest.time`(기본 21:30)에 지난 정리 이후 채널에 올라간 딜 중 **점수 높은 순으로 `max_items`(기본 10)건**을 골라 블로그 글 한 편을 만들어 관리자 챗으로 보냅니다. 양식은 Deren 블로그 규칙(`templates/blog_daily.j2`)을 따릅니다: 도입부 고정 2줄 → 채널 초대 블록 → 주제 도입 → 딜마다 "OO에서 OO이 O원입니다!" + 정가·시중가 대비·리뷰·배송 + 구매 링크 → 주의사항(느낌표 없이) → 마무리 → 대가성·고지 문구 → 면책 → 서명블록. 제목은 본문과 따로 **후보 3개**가 안내 메시지에 오고, 태그는 별도 상자로 옵니다. `/blog` 를 보내면 지금까지 모인 것으로 미리 만들어 볼 수 있습니다(이때는 기준 시각을 옮기지 않아 밤에 하루치가 다시 옵니다). 도입부의 블로그 이름·필명은 `blog_digest.blog_name/author`, 채널 이름은 `channels.telegram_name/kakao_openchat_name` 입니다.
