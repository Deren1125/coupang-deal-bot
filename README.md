...
| 발행처 | **자동**: 텔레그램 채널 + 스레드(Threads API) + 인스타그램(카드 이미지, Instagram API). 봇의 공개 도메인에 최근 딜 페이지(`/`, 인스타 프로필 링크용)와 딜별 바로가기(`/d/번호`). **반자동**: 카카오 오픈채팅·네이버 블로그용 완성 문구를 관리자 챗으로 보내면 복사해서 붙여넣기. 양식은 플랫폼별 템플릿(`deal_post.j2` / `deal_threads.j2` / `deal_kakao.j2` / `deal_blog.j2`)으로 각각 수정 |
| 품절 처리 | 게시판에 품절·종료 표시가 붙으면 기다리는 글을 내리고, 보냈던 "링크 필요" 메시지를 "품절되어 취소"로 고치고, 올린 지 24시간 안의 채널 글에는 "⛔ 품절" 표시. 토스·네이버는 링크 요청 전·발행 전에 상품 페이지 재고를 확인하고 링크를 기다리는 동안 5분마다 재확인 |
...
| `/instaauth` `/instacode 코드` `/instatest` | 인스타그램 연결 (최초 1회) · 샘플 글을 실제로 올려 카드·본문 확인 |
