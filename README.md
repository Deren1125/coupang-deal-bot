...
| | 내용 |
|---|---|
| 수집원 | 쿠팡파트너스 API(골드박스, 카테고리 베스트), **뽐뿌 핫딜 게시판(쿠팡·토스·네이버·11번가·G마켓·알리·올리브영·컬리… 태그 전부)**, 애드픽 핫딜 API(선택), 관리자 직접 입력 |
| 판정 | 모든 수집 상품의 가격을 SQLite 에 쌓고 (a) 표시 할인율 (b) 30일 평균가 대비 (c) 커뮤니티 추천 수 중 하나라도 만족하면 특가. 가격 없는 쿠폰/이벤트는 (c) 로만 |
| 링크 | 쿠팡: 파트너스 딥링크 API 자동. 11번가/G마켓/옥션/SSG/롯데온/알리/오늘의집: 링크프라이스 API 자동. 토스/네이버/올리브영/컬리/무신사: API 가 없어 **봇이 알려주면 내가 앱에서 만든 링크를 답장** (반자동). 그 외: 원본 링크 |
| 발행처 | 텔레그램 채널 1곳. 양식은 `templates/deal_post.j2` 로 자유 수정 |
| 모니터링 | 관리자 챗에 발행 성공/실패, 링크 요청, 에러, 일일 요약. `/status` 등 명령 |
...
텔레그램 값만 있어도 뽐뿌 수집 + 수동 발행이 됩니다. 쿠팡 키가 없으면 쿠팡 수집기와 딥링크가 꺼지고, 링크프라이스 ID 가 없으면 해당 몰은 원본 링크로 발행됩니다(`publish.allow_raw_links: false` 면 건너뜀).
...
- 쿠팡·링크프라이스 API 는 호출 제한이 있습니다. 기본 주기는 보수적으로 잡았습니다.
