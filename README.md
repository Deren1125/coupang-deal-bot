...
| 판정 | **관심도 게이트**(추천/댓글/조회수/순위)를 넘은 딜만 대상으로, **(d) 쿠팡 검색으로 찾은 같은 상품보다 10%↓** 를 기본 규칙으로 판정. 쿠팡보다 비싸면 탈락. 대조 결과가 없을 때만 (b) 30일 평균가 대비 15%↓, (c) 추천 5↑, (a) 표시 할인율 50%↑(서브) 로 판정. 가격 없는 쿠폰/이벤트/공지 글은 기본 설정에서 올리지 않음 (`deal.accept_coupons_and_events`) |
