...
| | 내용 |
|---|---|
| 수집원 | 쿠팡파트너스 API(골드박스, 카테고리 베스트), **뽐뿌 핫딜 게시판(쿠팡·토스·네이버·11번가·G마켓·알리·올리브영·컬리… 태그 전부)**, 알구몬 통합 핫딜(선택), 애드픽 핫딜 API(선택), 관리자 직접 입력 |
| 판정 | 모든 수집 상품의 가격을 SQLite 에 쌓고 (a) 표시 할인율 50%↑ (b) 30일 평균가 대비 15%↓ (c) 커뮤니티 추천 5↑ (d) **쿠팡 검색으로 찾은 같은 상품보다 10%↓** 중 하나라도 만족하면 특가. 쿠팡보다 비싼 딜은 탈락. 가격 없는 쿠폰/이벤트는 (c) 로만 |
...
| `/ppstats` | 뽐뿌 등 커뮤니티 글의 추천 수 분포 (규칙 (c) 임계값이 필터 구실을 하는지 확인) |
